...
### `ENABLE_SEARCH` <Badge text="可选" />

是否启用帖子/状态搜索。设置为 `false` 时，禁用 `/api/v2/search` 上的全文及状态搜索。

默认启用（`true`）。

### `ENABLE_ACCOUNT_SEARCH` <Badge text="可选" />

是否启用账户搜索。设置为 `false` 时，禁用 `/api/v1/accounts/search` 及 `/api/v2/search` 上的账户搜索。

默认启用（`true`）。

### `SHOW_REBLOGS` <Badge text="可选" />

是否在主时间线显示转推/分享的帖子（reblog、boost、renote）。设置为 `false` 时，在主时间线全局隐藏所有转推帖子。
