...
### `ENABLE_SEARCH` <Badge text="선택 사항" />

게시글/상태 검색 활성화 여부. `false`로 설정하면 `/api/v2/search`의 전문 검색 및 상태 검색이 비활성화됩니다.

기본값은 켜짐(`true`)입니다.

### `ENABLE_ACCOUNT_SEARCH` <Badge text="선택 사항" />

계정 검색 활성화 여부. `false`로 설정하면 `/api/v1/accounts/search` 및 `/api/v2/search`의 계정 검색이 비활성화됩니다.

기본값은 켜짐(`true`)입니다.

### `SHOW_REBLOGS` <Badge text="선택 사항" />

홈 타임라인에 공유된 게시글(리포스트, 부스트, 리노트) 표시 여부. `false`로 설정하면 홈 타임라인에서 모든 부스트가 전역적으로 숨겨집니다.
