...
### `ENABLE_SEARCH` <Badge text="オプション" />

投稿・ステータス検索を有効にするかどうか。`false` に設定すると、`/api/v2/search` での全文検索およびステータス検索が無効化されます。

基本的にはオン（`true`）になっています。

### `ENABLE_ACCOUNT_SEARCH` <Badge text="オプション" />

アカウント検索を有効にするかどうか。`false` に設定すると、`/api/v1/accounts/search` および `/api/v2/search` でのアカウント検索が無効化されます。

基本的にはオン（`true`）になっています。

### `SHOW_REBLOGS` <Badge text="オプション" />

ホームタイムラインに共有された投稿（リポスト、ブースト、リノート）を表示するかどうか。`false` に設定すると、グローバルにすべてのブーストがホームタイムラインから非表示になります。
