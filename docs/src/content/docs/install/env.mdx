...
Search and timeline settings
----------------------------

### `ENABLE_SEARCH` <Badge text="Optional" />

Whether to enable post/status search.  Set to `false` to disable full-text and
status search on `/api/v2/search`.

Turned on (`true`) by default.

### `ENABLE_ACCOUNT_SEARCH` <Badge text="Optional" />

Whether to enable account search.  Set to `false` to disable searching accounts
on `/api/v1/accounts/search` and `/api/v2/search`.

Turned on (`true`) by default.

### `SHOW_REBLOGS` <Badge text="Optional" />

Whether to show shared posts (reblogs, boosts, renotes) in the home timeline.
Set to `false` to hide all boosts from the home timeline globally.
